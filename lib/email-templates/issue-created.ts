...
export function getIssueCreatedEmailTemplate(
  issue: IssueData,
  dashboardUrl: string
): string {
  return `
        ...
        <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border: 2px solid #06b6d4; border-radius: 12px; padding: 40px;">
          ...
          <div style="line-height: 1.8; color: #ffffff;">
            ...
            <div style="background-color: #0f172a; padding: 25px; border-radius: 8px; margin: 25px 0; border: 1px solid #334155;">
              <h3 style="color: #06b6d4; margin-top: 0; font-size: 22px; margin-bottom: 15px;">${
                issue.title
              }</h3>
              <div style="margin: 15px 0;">
                <span style="display: inline-block; padding: 6px 14px; border-radius: 12px; font-size: 13px; font-weight: bold; margin-right: 8px; background-color: #14b8a6; color: #000000;">${
                  issue.type
                }</span>
                ${
                  issue.priority
                    ? `<span style="display: inline-block; padding: 6px 14px; border-radius: 12px; font-size: 13px; font-weight: bold; margin-right: 8px; background-color: #f97316; color: #000000;">${issue.priority}</span>`
                    : ""
                }
                ${
                  issue.status
                    ? `<span style="display: inline-block; padding: 6px 14px; border-radius: 12px; font-size: 13px; font-weight: bold; margin-right: 8px; background-color: #3b82f6; color: #ffffff;">${issue.status}</span>`
                    : ""
                }
              </div>
              <p style="color: #e5e7eb; font-size: 15px; line-height: 1.6; margin-top: 15px;">${
                issue.description
              }</p>
            </div>
